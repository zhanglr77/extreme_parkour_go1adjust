...
# play3.py: 只在15cm缝隙地形上跑，其他要求与play2一致
...
def play(args):
    ...
    # ⭐ 地形配置 - 创建2×1的15cm缝隙地形（避免除零错误）
    env_cfg.terrain.num_rows = 2  # 2行避免除零错误
    env_cfg.terrain.num_cols = 1  # 只有1列
    env_cfg.terrain.terrain_length = 8.0  # 8m地形
    env_cfg.terrain.terrain_width = 4.0   # 4m宽度
    env_cfg.terrain.horizontal_scale = 0.05  # 5cm网格
    ...
    env_cfg.terrain.terrain_dict = {
        ...
        "parkour_gap": 0.0,        
        "demo": 0.0,
        "fixed_gap_5cm": 0.0,      # 不使用5cm缝隙
        "fixed_gap_15cm": 1.0      # 100%使用15cm固定缝隙
    }
    ...
    # ⭐ 两个机器人都使用15cm缝隙地形，但在不同难度
    import torch
    env.terrain_levels[0] = 0  # 第一个机器人在row0（难度0）
    env.terrain_levels[1] = 1  # 第二个机器人在row1（难度1）
    env.terrain_types[:] = 0   # 所有机器人都用col0（fixed_gap_15cm）
    ...
    for i in range(10*int(env.max_episode_length)):
        ...
        if i % 50 == 0:  # 每50步输出一次状态
            ...
            print(f"🤖 机器人0 (row0-15cm): 速度 {env.base_lin_vel[0, 0].item():.2f}m/s, 奖励 {rews[0].item():.2f}")
            print(f"🤖 机器人1 (row1-15cm): 速度 {env.base_lin_vel[1, 0].item():.2f}m/s, 奖励 {rews[1].item():.2f}")
            print("-" * 60)
